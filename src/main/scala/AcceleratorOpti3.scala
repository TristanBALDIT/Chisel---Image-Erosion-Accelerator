...
class AcceleratorOpti3 extends Module {
  ...
  val idle :: setBorder :: readCenter :: readInnerCross :: readOuterCross :: writeCross :: writeCorners :: done :: Nil =
    Enum(8)
  ...
  val previous_cross_buffers = RegInit(
    VecInit(Seq.fill(2)(VecInit(Seq.fill(2)(1.U(8.W)))))
  )
  ...
  val bot_diag_buffers = RegInit(
    VecInit(Seq.fill(2)(VecInit(Seq.fill(10)(VecInit(Seq.fill(2)(1.U(8.W)))))))
  )
  val top_actual_diag_buffer = RegInit(
    VecInit(Seq.fill(10)(VecInit(Seq.fill(2)(1.U(8.W)))))
  )
  ...
  val allCornerWrote =
    cornerWrote(0) && cornerWrote(1) && cornerWrote(2) && cornerWrote(3)
  ...
  val outer_useful = VecInit(
    Seq(
      cross_buffer(1) === 255.U || cross_buffer(4) === 255.U || (io
        .dataRead(7, 0) === 255.U && read_idx === 4.U),
      (cross_buffer(2) === 255.U || (cross_buffer(4) === 255.U && !(io
        .dataRead(7, 0) === 0.U && read_idx === 5.U)) || (io.dataRead(
        7,
        0
      ) === 255.U && read_idx === 4.U)) && previous_cross_buffers(
        previous_cross
      )(0) === 1.U,
      ((cross_buffer(2) === 255.U && !(io
        .dataRead(7, 0) === 0.U && read_idx === 6.U)) || cross_buffer(
        3
      ) === 255.U) && bot_diag_buffers(previous_line)(buffer_idx)(1) === 1.U,
      (cross_buffer(1) === 255.U && !(io
        .dataRead(7, 0) === 0.U && read_idx === 7.U)) || (cross_buffer(
        3
      ) === 255.U && !(io.dataRead(7, 0) === 0.U && read_idx === 7.U)),
      cross_buffer(1) === 255.U && !(io
        .dataRead(7, 0) === 0.U && (read_idx === 5.U || read_idx === 8.U)),
      cross_buffer(4) === 255.U && previous_cross_buffers(previous_cross)(
        1
      ) === 1.U && !(io
        .dataRead(5, 0) === 0.U && (read_idx === 6.U || read_idx === 6.U)),
      cross_buffer(2) === 255.U && bot_diag_buffers(previous_line)(buffer_idx)(
        0
      ) === 1.U && !(io
        .dataRead(7, 0) === 0.U && (read_idx === 6.U || read_idx === 7.U)),
      cross_buffer(3) === 255.U && top_actual_diag_buffer(buffer_idx + 1.U)(
        0
      ) === 1.U && !(io
        .dataRead(7, 0) === 0.U && (read_idx === 7.U || read_idx === 8.U))
    )
  )
  ...
  io.dataWrite := Mux(
    state === setBorder || state === writeCorners,
    0.U,
  ...
  switch(state) {
    ...
    is(writeCross) {
      ...
      // EXIT STATE
      when(
        write_idx === 4.U && (cross_center_x === 1.U || cross_center_x === 18.U || cross_center_y === 1.U || cross_center_y === 18.U)
      ) {
        state := setBorder
        ...
        write_idx === 4.U || (write_idx === 3.U && !cross_addresses_valid(
          write_idx + 1.U
        ))
      ) {
        state := readCenter
        ...
        when(cross_center_x < 18.U && cross_center_y < 19.U) {
          ...
          previous_cross_buffers := VecInit(
            Seq.fill(2)(VecInit(Seq.fill(2)(1.U(8.W))))
          )
          actual_cross := 0.U

          // UPDATE BOT LINE BUFFERS
          bot_diag_buffers(previous_line) := VecInit(
            Seq.fill(10)(VecInit(Seq.fill(2)(1.U(8.W))))
          )
    ...
    is(readInnerCross) {
      ...
      when(
        (cross_addresses_valid(read_idx + 1.U) && !cross_known(
          read_idx + 1.U
        )) && read_idx < 4.U
      ) {
        read_idx := read_idx + 1.U
      }.elsewhen(
        (cross_addresses_valid(read_idx + 2.U) && !cross_known(
          read_idx + 2.U
        )) && read_idx < 3.U
      ) {
        read_idx := read_idx + 2.U
      }.elsewhen(
        (cross_addresses_valid(read_idx + 3.U) && !cross_known(
          read_idx + 3.U
        )) && read_idx < 2.U
      ) {
    ...
    is(readOuterCross) {
      ...
      when(read_idx === 12.U) {
        ...
        !cross_addresses_valid(read_idx + 1.U) || !outer_useful(
          read_idx + 1.U - 5.U
        ) && read_idx < 11.U
      ) {
        when(
          !cross_addresses_valid(read_idx + 2.U) || !outer_useful(
            read_idx + 2.U - 5.U
          ) && read_idx < 10.U
        ) {
          when(
            !cross_addresses_valid(read_idx + 3.U) || !outer_useful(
              read_idx + 3.U - 5.U
            ) && read_idx < 9.U
          ) {
            when(
              !cross_addresses_valid(read_idx + 4.U) || !outer_useful(
                read_idx + 4.U - 5.U
              ) && read_idx < 8.U
            ) {
              state := writeCross
