...
class AcceleratorOpti2 extends Module {
  ...
  val centerRead :: leftRead :: rightRead :: topRead :: topLeftRead :: topRightRead :: extraLeftRead :: extraRightRead :: Nil =
    Enum(8)
      ...
      when(pxl_idx > 19.U) {
        ...
        when(io.dataRead(7, 0) === 0.U) {
          ...
          switch(buffer(line_bottom)(pxl_idx)) {
            ...
            is(255.U) {
              when(
                buffer(line_top)(pxl_idx - 1.U) === 255.U || buffer(line_top)(
                  pxl_idx - 1.U
                ) === 254.U
              ) {
                when(buffer(line_bottom)(pxl_idx - 1.U) === 1.U) {
                  ...
                  addressRead := addressRead + 1.U
                  pxl_idx     := pxl_idx + 1.U
                  when(buffer(line_bottom)(pxl_idx - 1.U) === 0.U) {
        ...
        when(
          buffer(line_top)(pxl_idx + 1.U) === 0.U || io.dataRead(7, 0) === 0.U
        ) {
          ...
          (buffer(line_bottom)(pxl_idx) === 254.U || buffer(line_bottom)(
            pxl_idx
          ) === 255.U) && pxl_idx > 0.U
        ) {
        ...
        when(
          buffer(line_top)(pxl_idx - 1.U) === 255.U && io.dataRead(7, 0) === 0.U
        ) {
          buffer(line_top)(pxl_idx - 1.U) := 254.U
        }
        ...
        when(
          io.dataRead(7, 0) === 0.U || buffer(line_top)(pxl_idx - 1.U) === 0.U
        ) {
